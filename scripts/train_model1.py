...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Model 1: EfficientNet spectrogram model.")
    parser.add_argument("--config", default="configs/model1_effnet_spectrogram.yaml")
...
def main() -> int:
    ...
    result = run_synthetic_training(
        ...
        lr=float(config.train.get("learning_rate", 3e-4)),
    )
    output_path = Path("reports/results/model1_smoke_metrics.json" if args.smoke else "reports/results/model1_run_metrics.json")
