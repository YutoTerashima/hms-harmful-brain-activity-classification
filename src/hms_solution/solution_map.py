...
"""Project-specific research helpers for HMS Harmful Brain Activity Classification.

Purpose: Model-family and notebook-to-pipeline mapping for the solution archive.
...
MODULE_PURPOSE = 'Model-family and notebook-to-pipeline mapping for the solution archive.'
...
def model_families(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def notebook_map(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = model_families(materialized)
...
def ensemble_components(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
