...
Purpose: Notebook integrity and archive inventory utilities.
...
MODULE_PURPOSE = 'Notebook integrity and archive inventory utilities.'
...
def list_notebooks(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def notebook_metadata(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = list_notebooks(materialized)
...
def inventory_table(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
